...
//		}
    ...
    public Status newfix(Timer timer, String logFile, int currentTry) throws IOException {
        String src = _subject.getHome() + _subject.getSsrc();
...
//								haveTryPatches.remove(replace);
    ...
    public Status fix(Timer timer, String logFile, int currentTry, String projectName, String bugId,Double guard, boolean numberLimited, boolean isFilter) throws IOException {
        String src = _subject.getHome() + _subject.getSsrc();
        List<Pair<String, Integer>> locations = _localization.getLocations(200);
        int correct = 0;
        Set<String> haveTryBuggySourceCode = new HashSet<>();
        ...
        for (Pair<String, Integer> loc : locations) {
            ...
            for (CodeBlock oneBuggyBlock : buggyBlockList) {
                ...
                List<Pair<CodeBlock, Double>> candidates = new ArrayList<>();
...
//                }
                candidates = simpleFilter.vectorFilter(src, projectName, bugId, guard, isFilter);

                logMessage(logFile, "Similar code counts for single buggyblock: " + candidates.size());
...
//                    modificationCount += modifications.size();
                    ...
                    while (true) {
                        for (Set<Integer> modifySet : list) {
                            ...
                            String replace = oneBuggyBlock.toSrcString().toString(); //补丁
                            if (replace.equals(currentBlockString)) {
                                for (Integer index : modifySet) {
                                    modifications.get(index).restore();
                                }
...
//                                    logFile = Constant.PROJ_LOG_BASE_PATH + "/" + projectName + "/" + bugId + ".log";
                                    ...
                                    for (int tmp = 0; tmp <= currentIndex; tmp++) {
                                        ...
                                        JavaFile.writeStringToFile(candidateLogFile, "Similar code rank: " + tmp + "\n", true);
                                        JavaFile.writeStringToFile(candidateLogFile, "Similarity: " + candidates.get(tmp).getSecond() + "\n", true);
