...
if __name__ == '__main__':
    ...
    model = load_model()
    print("Running " + project_name + " " + bug_id)
    ...
    # 1. 创建文件对象
    f = open(base_url + '/dict_result.csv', 'w', encoding='utf-8')
   ...
   # sim_neighbor_diff = 0
    for i in range(0, len(dict)):
       # mins = 1000000000
       # if i == 0:
          #  diff_neighbor = np.linalg.norm(embedding_dict[dict[i][0]].detach().numpy() - embedding_dict[0].detach().numpy())
         #   if diff_neighbor == 0.0:
        #        diff_neighbor = 1
       # for tmp in range(0, i):
            # mins = min(float(F.normalize(embedding_dict[dict[i][0]]).mm(F.normalize(embedding_dict[dict[tmp][0]]).t())), mins)
          #  mins = min(np.linalg.norm(
           #     embedding_dict[dict[i][0]].detach().numpy() - embedding_dict[dict[tmp][0]].detach().numpy()), mins)
        
            # diff_neighbor = float(F.normalize(embedding_dict[dict[i][0]]).mm(F.normalize(embedding_dict[dict[i - 1][0]]).t()))
       ...
       # print(str(n) + " " + str(dict[i]) + " " + str(mins) + " " + str(diff_neighbor) + str(sim_neighbor_diff))
        ...
        # n = n + 1
        # csv_writer.writerow([str(i), str(dict[i][0]), str(dict[i][1]), str(mins), str(diff_neighbor), str(sim_neighbor_diff)])
        csv_writer.writerow([str(i), str(dict[i][0]), str(dict[i][1])])

    # dict_result = pd.DataFrame(dict)
    #
    # a = time.time()
